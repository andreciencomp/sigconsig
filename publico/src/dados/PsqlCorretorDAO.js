    ...
    async salvar(corretor){
        try{
            ...
            if(corretor.endereco){
                ...
                let {rows} = await pool.query(queryEndereco, [corretor.endereco.estado.id, corretor.endereco.cidade.id,
            ...
            if(corretor.contaBancaria){
                let queryContaBancaria = "insert into contas_bancarias (num_agencia, num_conta, digito) values ($1, $2, $3) returning id";
                const {rows} = await pool
                    .query(queryContaBancaria,[corretor.contaBancaria.numAgencia, corretor.contaBancaria.numConta, corretor.contaBancaria.digito]);
